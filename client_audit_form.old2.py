...
with st.form("client_audit_form"):
    ...
    security_tools = [
        ...
        ("email_filtering", "Email Filtering tools"),
        ("dns_filtering", "DNS Filtering tools"),
        ("backup_onsite", "Backup Storage - Onsite tools"),
        ("backup_offsite", "Backup Storage - Offsite tools"),
        ("cloud_hosting", "Cloud Hosting tools"),
        ("next_gen_av", "Next Gen AV tools"),
        ("edr_tools", "Endpoint Detection and Response tools"),
        ("firewall", "Firewall (hardware or software) tools"),
        ("hardware_inventory", "Hardware Asset Inventory or Discovery tools"),
        ("software_inventory", "Software Asset Inventory or Discovery tools"),
        ("saas_inventory", "SAAS Inventory or Discovery tools"),
        ("mfa_tools", "Multi-factor Authentication tools"),
        ("pam_tools", "Privileged Access Management (PAM) tools"),
        ("security_training", "Security Awareness Training tools"),
        ("vulnerability_scanning", "Vulnerability Scanning tools"),
        ("siem_tools", "SIEM tools"),
        ("soc_mdr", "SOC/MDR tools")
    ]
    ...
    for i, (key, tool_name) in enumerate(security_tools):
        with cols[i % num_cols]:
            ...
            security_responses[key] = st.radio("Status", ["Not Needed", "No", "Partial", "Yes"], key=key, label_visibility="collapsed")
    
    # Security Policies and Procedures
    st.markdown('<div class="section-header"><h2>📋 Security Policies and Procedures</h2></div>', unsafe_allow_html=True)
    
    st.markdown("**The Organization has:** *(Check all that apply)*")
    security_policies = {}
    security_policies['written_security_plan'] = st.checkbox("A regularly tested and updated Written Information Security Plan", key="written_security_plan")
    security_policies['incident_response_plan'] = st.checkbox("A regularly tested and updated Written Incident Response Plan", key="incident_response_plan")
    security_policies['ciso_designated'] = st.checkbox("A designated Chief Information Security Officer (or equivalent)", key="ciso_designated")
    security_policies['none_above'] = st.checkbox("None of the above", key="none_above")
