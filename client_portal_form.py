...
def get_utm_params():
    ...
    try:
        ...
        # Debug: Print raw query params type and content
        print(f"DEBUG: query_params type: {type(query_params)}")
        print(f"DEBUG: query_params content: {query_params}")
        ...
        for param in all_params:
            try:
                ...
                if hasattr(query_params, '__getitem__'):
                    value = query_params[param] if param in query_params else ''
                else:
                    # Method 2: get() method
                ...
                if isinstance(value, list):
                    utm_data[param] = value[0] if len(value) > 0 else ''
                elif value is None:
                    utm_data[param] = ''
                else:
                    utm_data[param] = str(value)
                    
                print(f"DEBUG: {param} = {utm_data[param]}")
                
            except Exception as e:
                print(f"DEBUG ERROR for {param}: {str(e)}")
                utm_data[param] = ''
        ...
        print(f"DEBUG: Exception in get_utm_params: {str(e)}")
        import traceback
        traceback.print_exc()
        return {
            'utm_source': '',
...
with st.form("client_audit_form"):
    ...
    # Security Tools Assessment - HIDDEN FOR CLIENT PORTAL
    # Clients only provide business information, not technical security details
    # Initialize all security variables with "Not Applicable" defaults
    
    client_patch_mgmt = "Not Applicable"
    client_mfa = "Not Applicable"
    client_edr = "Not Applicable"
    client_email_filtering = "Not Applicable"
    client_backup = "Not Applicable"
    client_password_mgr = "Not Applicable"
    client_dns_filtering = "Not Applicable"
    client_phishing_training = "Not Applicable"
    client_dlp = "Not Applicable"
    client_encryption = "Not Applicable"
    client_vuln_scanning = "Not Applicable"
    client_siem = "Not Applicable"
    client_privileged_access = "Not Applicable"
    client_network_segmentation = "Not Applicable"
...
if submitted:
    ...
    if required_fields_validation:
        ...
        form_data = {
            ...
            'utm_source': st.session_state.utm_params.get('utm_source', ''),
            'utm_medium': st.session_state.utm_params.get('utm_medium', ''),
            'utm_campaign': st.session_state.utm_params.get('utm_campaign', ''),
            'utm_term': st.session_state.utm_params.get('utm_term', ''),
            'utm_content': st.session_state.utm_params.get('utm_content', '')
        }
        ...
        # Generate filename based on completion method with UTM source
        utm_source = st.session_state.utm_params.get('utm_source', '')
        utm_suffix = f"_{utm_source}" if utm_source else ""
        
        if is_msp_only:
            filename = f"msp_security_assessment_{organization_name.replace(' ', '_')}{utm_suffix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        else:
            filename = f"client_audit_complete_{organization_name.replace(' ', '_')}{utm_suffix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
